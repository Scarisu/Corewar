...
/*   Updated: 2018/04/03 13:53:49 by rlecart          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include <vm.h>

int		find_ind_xor(t_corewar *d, t_reg *reg, int pc, int len)
{
	int		param;
...
void	op_xor_loop(t_corewar *d, t_reg *reg, t_ocp ocp, int pc)
{
	int		i;
	int		r[3];
	int		len[2];
	...
	while (++i < 3)
	{
		...
		if (ocp.p[i] == O_DIR && (len[0] = 4))
			...
			r[i] = find_ind_xor(d, reg, pc, len[0]);
